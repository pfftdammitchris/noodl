...
before(() => {
    com
        .loadFilesAsDocs({
        as: 'metadataDocs',
        dir: path.resolve(path.join(process.cwd(), 'dev/noodl-morph/__tests__/fixtures')),
        recursive: false,
    })
        .forEach((obj) => {
...
describe(com.coolGold('visitor utils'), () => {
    describe(com.italic('getValueFromRoot'), () => {
        it(`should get values from root`, () => {
            ...
            expect(v.getValueFromRoot('Style.textAlign.y')).to.have.property('value', 'center');
            expect(v.getValueFromRoot('Style.textAlign.x')).to.have.property('value', 'center');
        });
