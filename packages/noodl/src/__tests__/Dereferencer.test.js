...
describe(coolGold('Dereferencer'), () => {
    describe(`getLocalReference`, () => {
        it(`should dereference to the expected value`, () => {
            const ref = '..title';
            const page = noodl.root.get('EditProfile');
            const node = page.find((node) => node?.value === ref);
            const result = dereferencer.getLocalReference(node, { page });
    ...
    xdescribe(`Apply`, () => {
        ...
        let nameField;
        beforeEach(() => {
            Global = dereferencer.root.get('Global');
        ...
        it(`should apply the value to the path referenced in the key`, () => {
            ...
            const Global = dereferencer.root.Global;
            Global.setIn('currentUser.vertex.name.firstName'.split('.'), firstName);
            const page = dereferencer.pages.get('EditProfile');
            // prettier-ignore
