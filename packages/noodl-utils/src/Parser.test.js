...
describe(coolGold(`Parser`), () => {
    describe(italic(`appConfigUrl`), () => {
        it(`should support using a rootConfig object`, () => {
            expect(parse.appConfigUrl(rootConfig, 'web', 'test')).to.eq(parsedCadlEndpoint);
        });
        it(`should support using the baseUrl`, () => {
            expect(parse.appConfigUrl(rootConfig.cadlBaseUrl, rootConfig.cadlMain, '0.100d')).to.eq(`https://public.aitmed.com/cadl/meet3_0.100d/${rootConfig.cadlMain}`);
        });
    });
