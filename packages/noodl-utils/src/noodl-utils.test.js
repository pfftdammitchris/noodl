...
describe(coolGold('createEmitDataKey'), () => {
    ...
    it('should attach the dataObjects using a path', () => {
        const dataKey = n.createEmitDataKey({ var1: 'Global', var2: 'MeetingRoomCreate' }, [dataObject, pageObject, root]);
    ...
    it('should attach the dataObject using a string dataKey', () => {
        expect(n.createEmitDataKey('MeetingRoomCreate', [dataObject, pageObject, root])).to.eq(root.MeetingRoomCreate);
        expect(n.createEmitDataKey('genderInfo.value', [dataObject, pageObject, root])).to.eq('Female');
...
describe(coolGold('excludeIteratorVar'), () => {
    ...
    u.entries(tests).forEach(([testStr, expectedResult]) => {
        it(`should return ${u.white(expectedResult)} for ${testStr}`, () => {
            expect(n.excludeIteratorVar(testStr, iteratorVar)).to.eq(expectedResult);
        });
    });
});
...
//# sourceMappingURL=noodl-utils.test.js.map