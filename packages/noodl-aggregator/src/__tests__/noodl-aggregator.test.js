...
const cadlEndpointYml = fs.readFileSync(path.join(__dirname, './fixtures/cadlEndpoint.yml'), 'utf8');
const meet4dYml = fs.readFileSync(path.join(__dirname, './fixtures/meet4d.yml'), 'utf8');
    ...
    describe(com.italic(`init`), () => {
        ...
        it(`should load all the preload pages by default`, async () => {
            ...
            preloadPages.forEach((preloadPage) => {
                expect(aggregator.root.get(preloadPage)).to.exist;
            });
        });
        it(`should load all the pages by default`, async () => {
            mockAllPageRequests();
            await aggregator.init({ loadPreloadPages: false });
            pages.forEach((page) => expect(aggregator.root.get(page.replace('~/', ''))).to.exist);
        });
    });
    describe(com.italic(`extractAssets`), () => {
        it(`should extract the assets`, async () => {
            ...
            for (const asset of assets) {
                expect(asset).to.have.property('url', `${assetsUrl}${asset.filename}${asset.ext}`);
            }
        });
    });
});
