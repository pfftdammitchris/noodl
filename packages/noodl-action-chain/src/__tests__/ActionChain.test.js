...
import { getActionChain, getBuiltInAction, getEvalObjectAction, getPopUpAction, getPopUpDismissAction, getSaveObjectAction, getUpdateObjectAction, } from './helpers';
...
describe(coolGold(`ActionChain`), () => {
    ...
    describe(italic(`Execution`), () => {
        ...
        describe(`Injecting`, () => {
            ...
            describe(`When the action injecting is a popUp using "${chalk.magenta('wait')}"`, () => {
    ...
    describe(italic(`Observers`), () => {
        ...
        describe(magenta(`onAbortEnd`), () => {
            it(`should call ${magenta(`onAbortEnd`)} in the end of aborting`, async () => {
                const spy = sinon.spy();
                const ac = getActionChain({
                    actions: [getEvalObjectAction(), getBuiltInAction()],
                    trigger: 'onChange',
        ...
        describe(magenta(`onBeforeAbortAction`), () => {
            xit(`should be called right before calling the action's abort method`, () => {
                ...
                // @ts-expect-error
                ac.loader = (actions) => {
                    return actions.map((a) => createAction({ action: a, trigger: 'onClick' }));
                };
                ac.loadQueue();
            });
        });
